...
> **Project note (online-exam-system):** this app is Bootstrap 5 via Vite
> (`resources/js/app.js`, `resources/css/app.css`); the Tailwind plugin is
> commented out in `vite.config.js`, so Tailwind classes do not compile.
> Apply this skill's principles through Bootstrap utilities / custom CSS, not
> Tailwind. The exam-taker JS depends on `.question-card[data-question-id]`
> and `data-config` JSON — keep that markup contract intact.
