...
| R3 | Extract `ExamSessionService` + `GradingService` + `ViolationService`; server-side expiry. Absorbed from A: async grading/violation jobs, DTOs, custom exceptions | done | 50–80k | | Committed `2c892c1`. Completion: `exams:expire-sessions` cmd + everyMinute schedule + Feature test; dead Jobs deleted (recreate in R10 on Redis); P0.9 batch (generic submit/start errors, pivot-membership + subject checks on 5 mutations, teacher dashboard branch, dead `getDetailedResults` deleted); `begin()` persists scheduled→in_progress in `take()`; `forceEnd` guarded. Suite 30/30. P0.8 `DB` import folded in as drive-by (ExamController lines now execute). Follow-up (grill Q1/Q2): `take()` reverted to view-only, `POST begin` route+action added (lobby P0 fixed); `paused_at` migration + pause stamp + `resume()` clock-shift wired into teacher `resumeSession`; polling bridge deferred to R6. Suite 34/34 |
| R4 | Extract `ExamManagementService`; safe uploads. Absorbed from A: `FileService` | done | 30–45k | | Committed `2340a3d`. Test-first (16 cases). Controller 388→242, all actions delegate-only; same flash/404/422 shapes; `fresh()->total_marks` fix. `destroy()` deletes orphaned upload; `whereDoesntHave` subquery; readonly `ExamData` DTO. P0.8 + pivot checks had already landed in R3 follow-ups. Suite 51/51 |
| R5 | Extract `DashboardService` + ViewModels; fix N+1, cache availability. Absorbed from A: `Cache::remember` recipes, column selection, view composers | done | 30–45k | | Test-first (8 cases). Controllers 907→157 lines, blades untouched (arrays, not ViewModels). `withCount`/`withSum`/grouped attempts, availability cached 300s. Deleted 14 dead privates + dead `Admin\DashboardController`. Admin placeholder activity moved verbatim — R11 should replace with real events. Suite 59/59 |
